...
// TDZ (Uninitialization, 초기화 되지 않은 상태, 접근 불가)
// data 변수 선언 (값 할당하지 않음)
let data // JS엔진이 초기화(Initialization, undefined 할당. 개발자 의도 없음)
...
console.log(greet(undefined,24));
...
let currentUser = null // 로그인 안함 상태(시간의 흐름에 따라 변하는 데이터)
...
currentUser = null // 로그인 된 상태 > 로그아웃 상태로 변경
...
const user = {
  ...
  user.bio = '멋사 프론트엔드 부트캠프 14기 수강생으로 열심히 공부중입니다! 😁'
...
console.log(null == undefined) // 사용 ❌
console.log(!!null == !!undefined) // 사용 ❌
console.log(Boolean(null) == Boolean(undefined)) // 사용 ❌
